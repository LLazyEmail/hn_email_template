import require$$0, { readFileSync, existsSync, mkdirSync as mkdirSync$1 } from 'fs';
...
var CONST_FILE_NOT_WRITTEN = 'file not written';
var ERROR_NO_CONTENT = 'content variable is empty';

function writeHTML(fileName, content) {
  var dir = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "generated";
  var message = arguments.length > 3 ? arguments[3] : undefined;

  var _path = "".concat(dir, "/").concat(fileName);

  if (!content) {
    throw new Error(ERROR_NO_CONTENT);
  }

  write_1(_path, content).then(function () {
    message && console.log("file has been written successfully".concat(fileName));
  })["catch"](function (err) {
...
var generateTemplateName = function generateTemplateName(suffix) {
  var ext = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'html';
  return "".concat(suffix, "-").concat(Date.now(), ".").concat(ext);
};
...
var stateInit = function stateInit(source) {
  var markdown = readSourceFile(source);
  var stateObject = {
    content: markdown,
    previewText: '',
    warnings: {
    ...
    innerCheckErrors: function innerCheckErrors() {
      checkErrors(this.errors);
    },
    innerWarnings: function innerWarnings() {
...
function checkWarnings(warnings) {
  lodash.exports.forEach(warnings, function (index, element) {
    if (index) {
      var message = "WARNING source.md has ".concat(index, " ").concat(element, ". Replace it with memes");
...
function checkErrors(errors) {
  if (Object.values(errors).includes(false)) {
    lodash.exports.forEach(errors, function (_, error) {
      if (!errors[error]) {
        var _message = "ERROR source.md doesn't have ".concat(error);
...
function displayCLIErrors(errors, warnings) {
  if (checkErrors(errors)) ; else {
    checkWarnings(warnings);
  }
}

function checkHtml(content) {
  var searchPattern = new RegExp('(<table|<tr>|<td|<tbody>)', 'i');
  var ind;
  var tempStr = '';

  if (process$1.env.noAdv) {
    ind = content.indexOf('<h1 class="mc-toc-title"');
  } else {
    ind = content.indexOf('</table></span></span></div>');
  }

  for (var i = ind; i < ind + 5000; i++) {
    tempStr += content[i];
  }

  if (searchPattern.test(tempStr)) {
    printMessage('Content has correct html!!!');
  } else {
    printMessage('Content has not correct html!!!');
  }
}
...
var printMessage = function printMessage(_ref) {
  ...
  switch (type) {
    ...
    case 'red2':
      console.log(source.red.bold(message));
      break;
    ...
    case 'green2':
      console.log(source.green.bold(message));
      break;
...
export { checkErrors, checkHtml, checkWarnings, displayCLIErrors, generateTemplateName, isFolderExists, printMessage, readSourceFile, stateInit, writeHTML };
