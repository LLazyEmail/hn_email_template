/**
 * Runtime validation utilities for the innerComponents module.
 *
...
/**
 * Dedicated error type for innerComponents validation failures.
 *
 * Message format:
 *   [innerComponents.<context>] <description>. Expected: <expected>. Received: <received>.
 */
export class InnerComponentsValidationError extends Error {
  constructor(message) {
    super(message);
    this.name = 'InnerComponentsValidationError';
  }
}
...
export function assertRequired(context, field, value) {
  if (value === undefined || value === null) {
    throw new InnerComponentsValidationError(
      `[innerComponents.${context}] Missing required param "${field}". Expected a value. Received: ${value}.`
    );
  }
}
...
export function assertNonEmptyString(context, field, value) {
  ...
  if (typeof value !== 'string' || value.trim() === '') {
    throw new InnerComponentsValidationError(
      `[innerComponents.${context}] Invalid param "${field}". Expected a non-empty string. Received: ${JSON.stringify(value)}.`
    );
  }
}
